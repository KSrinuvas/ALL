...
my ($diff_cell1,$diff_cell2);
foreach (@keys)	{
		if ($_ eq 'cell1')	{
				if ($hash1{$_} < $hash2{$_})	{
						$diff_cell1 = $hash2{$_} - $hash1{$_};
				} elsif ($hash1{$_} > $hash2{$_})	{
						$diff_cell1 = $hash1{$_} - $hash2{$_};
				}
		} elsif ($_ eq 'cell2')	{
				if ($hash1{$_} < $hash2{$_})	{
						$diff_cell2 = $hash2{$_} - $hash1{$_};
				} elsif ($hash1{$_} > $hash2{$_})	{
						$diff_cell2 = $hash1{$_} - $hash2{$_};
				}
		}
}
...
print"differance of cell1 is : $diff_cell1\n";				
print"differance of cell2 is : $diff_cell2\n";				
